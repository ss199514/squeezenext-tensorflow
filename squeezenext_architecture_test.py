...
def squeezenext_unit(inputs, filters, stride,height_first_order, groups,seperate_relus):
    ...
    block = tf.keras.layers.Conv2D(inputs, filters // 2, [1, 1], groups, strides=stride)
    ...
    if height_first_order:
        ...
        block = tf.keras.layers.Conv2D(block, input_channels_seperated * 2, [3, 1], groups)
        block = tf.keras.layers.Conv2D(block, block.get_shape().as_list()[-1], [1, 3], groups)

    else:
        input_channels_seperated = block.get_shape().as_list()[-1]
        block = tf.keras.layers.Conv2D(block, input_channels_seperated * 2, [1, 3], groups)
        block = tf.keras.layers.Conv2D(block, block.get_shape().as_list()[-1], [3, 1], groups)
...
class SqueezeNext(object):
    ...
    def __call__(self, inputs, training,height_first_order = True):
        ...
        with tf.compat.v1.variable_scope("squeezenext"):
            ...
            for block_idx,block_def in enumerate(self.block_defs):
                ...
                with tf.compat.v1.variable_scope("block_{}".format(block_idx)):
                    # create seperate units inside a block
                    for unit_idx in range(0,units):
                        with tf.compat.v1.variable_scope("unit_{}".format(unit_idx)):
                            if unit_idx != 0:
...
def squeeze_next_arg_scope(is_training,
    ...
    weights_init = tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")
    regularizer = tf.keras.regularizers.l2(0.5 * (weight_decay))


    with slim.arg_scope([slim.conv2d,tf.keras.layers.Conv2D],
